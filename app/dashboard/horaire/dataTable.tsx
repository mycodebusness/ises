...
import { Horaire as Payment } from "@/lib/types";
...
import { deleteHoraire } from "@/lib/actions";

export const columns: ColumnDef<Payment>[] = [
  {
    accessorKey: "id_horaire",
    header: "Id",
    cell: ({ row }) => (
      <div className="capitalize">{row.original.id_horaire}</div>
    ),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize">{row.original.jour || ""}</div>
    ),
  },
  // {
...

  {
    ...
    header: "Heure début",
    cell: ({ row }) => (
      <div className="capitalize">{row.original.heure_debut}</div>
    ),
  },
  {
    ...
    header: "Heure fin",
    cell: ({ row }) => (
      <div className="capitalize">{row.original.heure_fin}</div>
    ),
  },
      ...
      return (
        <div className="flex gap-3">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Link
                  ...
                  href={`/dashboard/horaire/ajouter`}
                >
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Link
                  className="flex gap-3"
                  href={`/dashboard/horaire/${row.original.id_horaire}`}
                >
              ...
              <TooltipContent className="flex gap-2 justify-center items-center">
                ...
                <p>modifier l&apos;horaire {row.original.id_horaire}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <Dialog>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="flex gap-2 justify-center items-center">
                  ...
                  <p>supprimer l&apos;horaire {row.original.id_horaire}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle> Suppression horaire</DialogTitle>
                <DialogDescription>
                  Etes-vous sur de supprimer définitivement l&apos;horaire
                  {row.original.id_horaire} ?
                </DialogDescription>
              </DialogHeader>
    ...
    <div className="w-full px-4 pt-4">
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <div className="flex-1 text-sm text-muted-foreground">
          Total : {table.getFilteredRowModel().rows.length} horaire(s).
        </div>
